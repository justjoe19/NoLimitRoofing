...
**Fix**: added `.main-nav .nav-dropdown-panel a[aria-current="page"] { text-accent-dark }` inside the existing desktop media query — specificity `(0,3,1)` unambiguously wins, and using `accent-dark` (same as the hover color) rather than a repeat of the plain resting `text-ink` means the current page still reads as visually marked, not just "fixed to not be broken." Verified in the compiled CSS output and live in the browser (navigated to a service page, reopened the dropdown, confirmed "Roof Replacement" now renders in orange with its underline, not white).

While fixing it, checked for the same shape elsewhere before waiting for another report: the mega-menu's group titles ("Roofing" / "Storm Damage" / "Commercial") are *also* real `<a>` links with their own `aria-current` (Header.astro sets it from `isActive(group.href)`), and the exact same generic rule would have made a group title invisible on desktop whenever its own hub page (e.g. `/roofing.html`) was the current page — an undiscovered fifth instance of the pattern, not yet reported by the user. Fixed proactively with `.nav-dropdown-panel--mega .nav-dropdown-group-title[aria-current="page"] { text-ink }`, verified the same way (compiled CSS + live browser check on `/roofing.html`, "Roofing" group title now renders in orange/underlined instead of white).

Added a standing gotcha to `README.md`'s Known Gotchas after three instances of this same bug shape: any nav rule keyed off `[aria-current="page"]` or `:hover` needs its own explicit color for *every* background context the element can appear in (dark top bar, dark mobile sheet, light desktop popover) — a rule written for one context will silently win by specificity in another.

## Session 36 — Desktop mega-menu group titles washed out on hover: another light-on-light gap, this time in reverse

User reported the nav link colors still had issues after Session 35's mobile fix. Different bug, same shape: `.main-nav .nav-dropdown-group-title:hover` (the "Roofing" / "Storm Damage" / "Commercial" bold headers in the Services mega-menu) uses `text-accent-light` (`#f0813a`) as its only hover rule — correct for mobile, where the panel sits on the dark `bg-ink` sheet (7.14:1 contrast), but on desktop the panel is the light `bg-paper` popover, and `accent-light` only measures 2.66:1 there — a real WCAG AA failure, confirmed by computing contrast directly rather than eyeballing it. There was a desktop override for the *resting* color (`text-accent-dark`, line 242) but nothing for `:hover`, so hovering a group title on desktop made it wash out instead of darkening.

**Fix**: added `.nav-dropdown-panel--mega .nav-dropdown-group-title:hover { text-ink }` inside the existing `@media (min-width: 64rem)` block — 18.98:1 on white, unambiguous. Verified against the compiled CSS output (same method as Sessions 33 and 35): base/mobile rule still resolves to `color:var(--color-accent-light)`, desktop-scoped rule resolves to `color:var(--color-ink)`, confirming each context gets the color suited to its own background rather than one rule trying to serve both.

Also reviewed the two commits made outside this session since the last check-in (27 new AI-generated photos replacing duplicate stock images across services/locations/articles, plus 6 distinct SVG icons on the Areas page) — verified every queued image exists and every source-file replacement actually applied, zero duplicate photo references remain, clean build, 0 SEO/broken-link issues, 0 console errors.

Fixed three leftover mismatches from that batch that its generation script never touched (it only swaps `src` paths, never `alt`/`width`/`height`): `service-siding.webp`'s `alt` text still described the *old* image it replaced ("Completed home exterior roofing project" → "Home exterior with newly installed siding"), and `service-shingle-options.webp` / `storm-emergency-tarp.webp` had stale `width`/`height` attributes left over from their old images (both now correctly `800×450`, matching the real files — cosmetically inert since `.service-card .media` forces a fixed 4:3 box via CSS regardless, but wrong metadata is still wrong).

Left one pre-existing item alone: `service-roof-overlay.webp` on the Services page is the client's own branded before/after project-gallery graphic (logo + "BEFORE" thumbnail baked in) being used as a small grid thumbnail — reads a little busy at that size, but fixing it means sourcing or generating a different photo, not a mechanical bug fix, so noted rather than changed.

## Session 35 — Mobile nav sub-menu links were invisible: dark-on-dark, not a contrast tuning issue

User reported the dropdown sub-menu links (Roof Replacement, Hail Damage, etc.) were invisible in mobile mode. This was a leftover blind spot from Session 33's specificity fix, not a new regression: `.main-nav .nav-dropdown-panel a` was set to `text-ink` (`#0c1115`) as an unconditional base rule, correct for desktop where `.nav-dropdown-panel` becomes a light `bg-paper` popover (inside `@media (min-width: 64rem)`) — but below that breakpoint, the panel has no background of its own and just sits on `.main-nav`'s mobile sheet background, `bg-ink` (also `#0c1115`). Literally identical text and background color, confirmed by grepping the compiled CSS output rather than trusting a screenshot. The bold group titles above the links (`.nav-dropdown-group-title`) were already `text-white` and unaffected — matching the user's report that only the *links*, not the headers, had disappeared.

**Fix**: swapped the base (mobile-default) rule to `text-mist` (`#e4e7eb`, ~17:1 on `#0c1115`) resting / `text-accent-light` hover — both already-used tokens for light-on-dark contexts elsewhere in the nav. Moved the desktop-specific `text-ink` / `text-accent-dark` pair into the existing `@media (min-width: 64rem)` block instead of leaving it as the unconditional default, so desktop (light popover) and mobile (dark sheet) each get colors suited to their own background. Verified against the compiled `dist/_astro/*.css` output directly: the unconditional rule now reads `color:var(--color-mist)`, and `color:var(--color-ink)` only exists inside the `width>=64rem` media block — confirming the fix applies exactly where intended, since the browser-resize tool remains unreliable for visually confirming true mobile viewports in this project.

Also hit the same stale-`dist/` artifact from Session 32 mid-verification (50 pages instead of 48, from a leftover build racing a running server) — `rm -rf dist && npm run build` cleared it again. 0 SEO/broken-link issues across the correct 48 pages afterward.
