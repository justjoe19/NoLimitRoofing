...
- **A single color rule serving two different-background contexts is a recurring failure pattern in this nav.** Three separate real bugs (Sessions 32-33 and 36) all had the same shape: one color rule was written as if only one background existed, but the same element renders on a light background in one context (desktop dropdown popover) and a dark one in another (mobile nav sheet, or vice versa for hover states). Whenever a nav rule's resting *and* hover colors aren't both explicitly set for *both* the mobile and desktop context, assume it's wrong until proven otherwise — verify with computed styles or the compiled CSS output, not a single screenshot.
