---
title: "What Happens If Bad Decking Is Found?"
metaDescription: "What it means when a roof tear-off reveals damaged decking, why it happens, and how it's actually handled — no surprises, no scare tactics."
publishDate: 2026-07-21
excerpt: "It's more common than you'd think, and it doesn't have to derail your project if it's handled honestly."
heroImage: "/images/article-bad-roof-decking-hero.webp"
faqs:
  - question: "How common is it to find bad decking during a tear-off?"
    answer: "It's common enough that a trustworthy contractor addresses it upfront in the estimate as a real possibility, not a rare surprise. Whether it happens on your specific roof depends on its age, history, and ventilation, not chance."
  - question: "Can I skip decking replacement to save money?"
    answer: "No — installing new roofing over damaged decking doesn't fix the underlying problem, it just hides it under new material until it fails again, often sooner and with more consequential damage. Damaged decking has to be addressed for the new roof to actually perform as intended."
  - question: "Will I be shown the damaged decking before it's replaced?"
    answer: "Yes — that's the standard we hold every job to. You should see the actual damage and understand the pricing before the work happens, not just be told about it after the fact."
  - question: "Does decking replacement affect my roof's warranty?"
    answer: "No, properly replacing damaged decking before installing new roofing is a normal, expected part of a correct installation — it supports the warranty rather than threatening it. Installing over bad decking would be the actual warranty risk."
draft: false
---

Decking is the wood layer underneath your shingles, underlayment, and ice and water shield — it's the structural surface everything else is attached to. It's also the one part of a roofing system you genuinely can't fully evaluate until the old roofing material comes off, which is why "we found some bad decking" is one of the more common things homeowners hear mid-project. Here's what it actually means.

## What causes roof decking to fail

Long-term moisture intrusion — from an old leak, ice damming, or age-related wear in ventilation, all common in Michiana's freeze-thaw climate — is the most common cause. Decking can also be damaged by wood rot, insect activity, or simply age in an older home. None of this is usually visible from inside the attic unless the damage is already severe.

## How bad decking is found during tear-off

During tear-off, the crew walks the deck and checks for soft spots, visible rot, delamination (plywood layers separating), or sagging. This is a normal, expected part of any full roof replacement — checking the decking isn't optional, because installing new roofing over bad decking just seals the problem in rather than fixing it.

## What happens after bad decking is found

Damaged decking sections get cut out and replaced with new material before the new roofing system goes on. This is priced separately from the base roofing estimate, specifically because it can't be accurately assessed — or priced — until the old roof is actually off. A trustworthy contractor will show you the damage, explain what's being replaced and why, and give you a clear price before doing the work, not just add a surprise line item to the final bill.

## Types of decking damage, and what causes each

Soft spots usually mean sustained moisture exposure over time — often from a slow, previously unnoticed leak. Delamination (plywood layers separating) is frequently age and moisture-cycling related, especially in older homes. Sagging can indicate structural stress beyond simple moisture damage, and gets flagged for a closer look at what's causing it, not just patched over. Insect activity, while less common, is also occasionally found and needs to be addressed as its own issue alongside the decking repair.

## How decking replacement is priced

Because the extent of damage can't be known until tear-off, decking replacement is typically priced as a separate line item based on the actual square footage of material replaced, once it's visible — not folded invisibly into the base roofing estimate. This is standard practice across the industry, not something specific to a low-cost or high-cost contractor; the difference between contractors is whether that pricing is transparent and shown to you, or added quietly to the final bill.

## How to avoid a surprise

The honest answer is: you can't fully avoid the possibility, since some decking issues genuinely aren't visible until tear-off. What you can do is work with a contractor who tells you upfront that this is a possibility, prices it transparently if it happens, and shows you the actual damage rather than just telling you about it. That's the standard we hold every job to — if decking work is needed on your project, you'll see it and understand why before we do it. Call (574) 360-0525 or request a free inspection to get started.
